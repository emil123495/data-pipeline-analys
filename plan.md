# 🌤️ Weather Forecast Accuracy Pipeline (v2 — jedno źródło API)
...
**Kluczowe uproszczenie względem wcześniejszej wersji:** nie ma osobnego wywołania do Archive/Historical API. Wszystko pochodzi z **jednego** endpointu (Forecast API), odpytywanego cyklicznie. Rekord z najmniejszym lead_time z każdego zapytania pełni rolę "danych rzeczywistych" (nowcast).

**Ważne zastrzeżenie metodologiczne (do README):**
> Jako "dane rzeczywiste" przyjmuję wyjście modelu pogodowego z najkrótszym możliwym wyprzedzeniem, a nie pomiary z fizycznych stacji meteorologicznych. To świadome uproszczenie — pozwala uniknąć zależności od opóźnionego API archiwalnego i uprościć architekturę do jednego źródła danych. Efekt jest bardzo zbliżony do rzeczywistości, ale metodologicznie to nie to samo.

**Źródło danych:** [Open-Meteo Forecast API](https://open-meteo.com/) — darmowe, bez klucza autoryzacyjnego

**Stack technologiczny:** Python, Pandas, SQLite → PostgreSQL (Docker), Streamlit, cron/Airflow/GitHub Actions, (opcjonalnie) AWS S3
...
4. Wszystkie rekordy (łącznie z tym o lead_time=0) trafiają do `fact_forecast`
...
- [ ] Timeout (`timeout=10`) + obsługa błędów per miasto, logowanie
- [ ] Zapis surowego JSON-a skompresowanego (`.json.gz`) z timestampem pobrania w nazwie
...
- [ ] Streamlit: wykres — prognozowana vs rzeczywista temperatura w czasie, dla wybranego miasta i wybranego lead_time
- [ ] Streamlit: porównanie miast — gdzie prognozy są najdokładniejsze
...
- [ ] Zapis surowych plików do AWS S3 zamiast lokalnego folderu
- [ ] Retencja raw danych (np. czyszczenie/archiwizacja plików >30 dni)
- [ ] Orkiestracja: Airflow (jeden DAG: fetch → validate → transform → split → load) lub GitHub Actions z harmonogramem godzinowym
- [ ] Retry logic (np. 3 próby co 5 minut przy błędzie API)
...
## Harmonogram orientacyjny (~2-3 tygodnie)

| Dni | Zadanie |
|---|---|
| 1-2 | Konfiguracja miast, `fetch_weather()`, zapis raw |
| 3-4 | Transform: `lead_time_hours`, walidacja, rozdział forecast/actual |
| 5-6 | Schemat bazy, load z `UNIQUE`/`INSERT OR IGNORE`, test idempotencji |
| 7 | Odpalenie zbierania danych "w tle" (cron co godzinę), żeby dane zaczęły się kumulować |
| 8-9 | Zapytanie łączące forecast z actual, pierwsze wyniki błędu prognozy (dla krótkich lead_time — dłuższe będą dojrzewać w czasie) |
| 10-11 | Dashboard Streamlit |
| 12-13 | (opcjonalnie) Docker + Postgres, retry logic |
| 14 | README, porządki, diagram architektury |

*Uwaga: pełna analiza dla lead_time = 7 dni wymaga fizycznie odczekania ~7 dni od startu zbierania danych. To naturalna cecha projektu — możesz go pokazać jako "żyjący", stale zbierający da